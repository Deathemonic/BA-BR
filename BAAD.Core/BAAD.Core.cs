    ...
    static void uniffiCheckApiChecksums() {
        {
            ...
            if (checksum != 3738) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_clear_all` checksum `3738`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_create_parent_dir();
            if (checksum != 52756) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_create_parent_dir` checksum `52756`, library returned `{checksum}`");
            }
        }
        {
        ...
        }
        {
            ...
            if (checksum != 22654) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_get_data_path` checksum `22654`, library returned `{checksum}`");
            }
        }
        {
        ...
        }
        {
            ...
            if (checksum != 34433) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_is_dir_empty` checksum `34433`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_load_file();
            if (checksum != 14605) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_load_file` checksum `14605`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_debug();
            if (checksum != 57349) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_debug` checksum `57349`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_debug_with_field();
            if (checksum != 29403) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_debug_with_field` checksum `29403`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_debug_with_fields();
            if (checksum != 45317) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_debug_with_fields` checksum `45317`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_error();
            if (checksum != 6284) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_error` checksum `6284`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_error_from_string();
            if (checksum != 6942) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_error_from_string` checksum `6942`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_error_with_field();
            if (checksum != 45353) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_error_with_field` checksum `45353`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_error_with_fields();
            if (checksum != 64124) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_error_with_fields` checksum `64124`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_info();
            if (checksum != 7152) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_info` checksum `7152`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_info_with_field();
            if (checksum != 8500) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_info_with_field` checksum `8500`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_info_with_fields();
            if (checksum != 2166) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_info_with_fields` checksum `2166`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_recoverable_error_from_string();
            if (checksum != 65090) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_recoverable_error_from_string` checksum `65090`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_trace();
            if (checksum != 25040) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_trace` checksum `25040`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_trace_with_field();
            if (checksum != 49472) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_trace_with_field` checksum `49472`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_trace_with_fields();
            if (checksum != 50270) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_trace_with_fields` checksum `50270`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_warn();
            if (checksum != 27846) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_warn` checksum `27846`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_warn_with_field();
            if (checksum != 39338) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_warn_with_field` checksum `39338`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_log_warn_with_fields();
            if (checksum != 63807) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_log_warn_with_fields` checksum `63807`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_save_file();
            if (checksum != 21264) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_save_file` checksum `21264`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_set_app_name();
            if (checksum != 6953) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_set_app_name` checksum `6953`, library returned `{checksum}`");
            }
        }
        {
            var checksum = _UniFFILib.uniffi_baad_core_checksum_func_set_data_dir();
            if (checksum != 17024) {
                throw new UniffiContractChecksumException($"BAAD.Core: uniffi bindings expected function `uniffi_baad_core_checksum_func_set_data_dir` checksum `17024`, library returned `{checksum}`");
            }
        }
    }
}
