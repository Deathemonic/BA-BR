...
public delegate RESULT DSP_PROCESS_CALLBACK(ref DSP_STATE dsp_state, uint length, ref DSP_BUFFER_ARRAY inbufferarray,
...
public delegate RESULT
    DSP_GETPARAM_FLOAT_CALLBACK(ref DSP_STATE dsp_state, int index, ref float value, IntPtr valuestr);
...
public delegate RESULT DSP_GETPARAM_DATA_CALLBACK(ref DSP_STATE dsp_state, int index, ref IntPtr data, ref uint length,
    IntPtr valuestr);
...
public delegate RESULT DSP_DFT_FFTREAL_FUNC(ref DSP_STATE dsp_state, int size, IntPtr signal, IntPtr dft, IntPtr window,
    int signalhop);
...
public struct DSP_METERING_INFO
{
    ...
    public struct LEVEL_ARRAY
    {
        ...
        {
            var buffer = new float[levels.Length];
            for (var i = 0; i < levels.Length; i++) buffer[i] = levels[i];
            return buffer;
        }
        ...
        {
            var len = buffer.Length >= Length ? Length : buffer.Length;
            for (var i = 0; i < len; i++) buffer[i] = this[i];
        }
    }
